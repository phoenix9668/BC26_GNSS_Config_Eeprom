...
//####################################################################################################
void atc_deinit(atc_t *atc, USART_TypeDef *USARTx)
{
  if (atc->inited == false)
    return;
  memset(atc, 0, sizeof(*atc));
	atc->usart = USARTx;
  LL_LPUART_DisableIT_RXNE(atc->usart);
  atc->inited = false;
  atc_printf("\r\n[%s] deinited.\r\n", atc->name);
}
//####################################################################################################
...
void atc_transmit(atc_t *atc, uint8_t *data, uint16_t len)
{
  ...
  {
    while (!LL_LPUART_IsActiveFlag_TXE(atc->usart))
      atc_delay(1);
    LL_LPUART_TransmitData8(atc->usart, data[i]);
  }
  while (!LL_LPUART_IsActiveFlag_TC(atc->usart))
    atc_delay(1);
}
//####################################################################################################
void atc_rxCallback(atc_t *atc)
{
  if (LL_LPUART_IsActiveFlag_RXNE(atc->usart))
  {
    uint8_t tmp = LL_LPUART_ReceiveData8(atc->usart);
